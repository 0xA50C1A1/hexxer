...
typedef enum {
	...
	MF_AMBUSH = 32,
	MF_JUSTHIT = 64, // try to attack right back
	MF_JUSTATTACKED = 128, // take at least one step before attacking
	MF_SPAWNCEILING = 256, // hang from ceiling instead of floor
	MF_NOGRAVITY = 512, // don't apply gravity every tic

	// movement flags
	MF_DROPOFF = 0x400, // allow jumps from high places
	MF_PICKUP = 0x800, // for players to pick up items
	...
	MF_TELEPORT = 0x8000, // don't cross lines or look at heights
	MF_MISSILE = 0x10000, // don't hit same species, explode on block

	MF_ALTSHADOW = 0x20000, // alternate fuzzy draw
	MF_SHADOW = 0x40000, // use fuzzy draw (shadow demons / invis)
	MF_NOBLOOD = 0x80000, // don't bleed when shot (use puff)
	MF_CORPSE = 0x100000, // don't stop moving halfway off a step
	MF_INFLOAT =
		0x200000, // floating to a height for a move, don't auto float to target's height

	MF_COUNTKILL = 0x400000, // count towards intermission kill total
	MF_ICECORPSE = 0x800000, // a frozen corpse (for blasting)

	MF_SKULLFLY = 0x1000000, // skull in flight
	MF_NOTDMATCH = 0x2000000, // don't spawn in death match (key cards)

	MF_TRANSLATION = 0x1c000000, // use a translation table (>>MF_TRANSHIFT)
...
typedef enum {
	...
	MF2_PASSMOBJ =
		0x00001000, // Enable z block checking.  If on, this flag will allow the mobj to pass over/under other mobjs.
	MF2_CANNOTPUSH = 0x00002000, // cannot push other pushable mobjs
	MF2_DROPPED = 0x00004000, // dropped by a demon
	...
	MF2_IMPACT = 0x00200000, // an MF_MISSILE mobj can activate \
	// SPAC_IMPACT
