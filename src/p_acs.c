...
static int (*PCodeCmds[])(void) = { CmdNOP,
				    CmdTerminate,
				    CmdSuspend,
				    CmdPushNumber,
				    CmdLSpec1,
...
void P_CheckACSStore(void)
{
	...
	for (store = ACSStore; store->map != 0; store++) {
		if (store->map == gamemap) {
			P_StartACS(store->script, 0, store->args, NULL, NULL,
				   0);
			if (NewScript) {
...
static boolean AddToACSStore(int map, int number, byte *args)
{
	...
	for (i = 0; ACSStore[i].map != 0; i++) {
		...
		if (index == -1 &&
		    ACSStore[i].map == -1) { // Remember first empty slot
			index = i;
		}
	}
...
static void ThingCount(int type, int tid)
{
	...
	if (tid) { // Count TID things
		...
		for (think = thinkercap.next; think != &thinkercap;
		     think = think->next) {
			if (think->function !=
			    P_MobjThinker) { // Not a mobj thinker
				continue;
			}
			mobj = (mobj_t *)think;
