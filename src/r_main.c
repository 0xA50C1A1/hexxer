...
void R_InitLightTables(void)
{
	...
	for (i = 0; i < LIGHTLEVELS; i++) {
		startmap = ((LIGHTLEVELS - 1 - i) * 2) * NUMCOLORMAPS /
			   LIGHTLEVELS;
...
void R_SetupFrame(player_t *player)
{
	...
	if (localQuakeHappening[displayplayer] && !paused) {
		...
		viewx += ((M_Random() % (intensity << 2)) - (intensity << 1))
			 << FRACBITS;
		viewy += ((M_Random() % (intensity << 2)) - (intensity << 1))
			 << FRACBITS;
	}
...
void R_RenderPlayerView(player_t *player)
{
	...
	if (localQuakeHappening[displayplayer] && gamestate == GS_LEVEL) {
		...
		R_RenderBSPNode(numnodes -
				1); // head node is the last node output
		players[displayplayer].mo->flags2 &= ~MF2_DONTDRAW;
	} else {
		R_RenderBSPNode(numnodes -
				1); // head node is the last node output
