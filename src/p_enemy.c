...
boolean P_CheckMeleeRange(mobj_t *actor)
{
	...
	if (mo->z >
	    actor->z + actor->height) { // Target is higher than the attacker
		return (false);
...
boolean P_CheckMissileRange(mobj_t *actor)
{
	...
	dist = (P_AproxDistance(actor->x - actor->target->x,
		...
		FRACBITS) -
	       64;
	if (!actor->info
		     ->meleestate) { // No melee attack, so fire more frequently
		dist -= 128;
	}
	if (dist > 200) {
		dist = 200;
	...
	for (;; actor->lastlook = (actor->lastlook + 1) & 3) {
		...
		if (player->mo->flags & MF_SHADOW) { // Player is invisible
			if ((P_AproxDistance(player->mo->x - actor->x,
					     player->mo->y - actor->y) >
			     2 * MELEERANGE) &&
			    P_AproxDistance(player->mo->momx,
					    player->mo->momy) <
...
void A_Chase(mobj_t *actor)
{
	...
	if (gameskill ==
	    sk_nightmare) { // Monsters move faster in nightmare mode
...
void A_MinotaurLook(mobj_t *actor)
{
	...
	if (deathmatch) // Quick search for players
	{
		for (i = 0; i < MAXPLAYERS; i++) {
			...
			if (mo->health <= 0)
				continue;
			dist = P_AproxDistance(actor->x - mo->x,
					       actor->y - mo->y);
...
void A_MinotaurAtk2(mobj_t *actor)
{
	...
	if (mo) {
		...
		momz = mo->momz;
		angle = mo->angle;
		P_SpawnMissileAngle(actor, MT_MNTRFX1, angle - (ANG45 / 8),
				    momz);
		P_SpawnMissileAngle(actor, MT_MNTRFX1, angle + (ANG45 / 8),
				    momz);
		P_SpawnMissileAngle(actor, MT_MNTRFX1, angle - (ANG45 / 16),
				    momz);
		P_SpawnMissileAngle(actor, MT_MNTRFX1, angle + (ANG45 / 16),
				    momz);
	}
	...
	if (actor->player) {
		if (actor->player->morphTics) {
			...
			if (actor->momz <= -39 * FRACUNIT) { // Falling splat
				...
				sound += P_Random() %
					 3; // Three different extreme deaths
			}
			S_StartSound(actor, sound);
		}
	} else {
...
void A_BishopMissileWeave(mobj_t *actor)
{
	...
	newY = actor->y -
	       FixedMul(finesine[angle], FloatBobOffsets[weaveXY] << 1);
	...
	if (dist < 4) { // Hit the target thing
		if (actor->target && P_Random() < 200) {
			...
			for (i = 0; i < 5; i++) {
				...
				search = -1;
				mo = P_FindMobjFromTID(target->args[i],
						       &search);
...
void A_IceGuyAttack(mobj_t *actor)
{
	...
	an = (actor->angle + ANG90) >> ANGLETOFINESHIFT;
	P_SpawnMissileXYZ(
		actor->x + FixedMul(actor->radius >> 1, finecosine[an]),
...
void A_SorcBallOrbit(mobj_t *actor)
{
	...
	if ((angle < prevangle) &&
	    (parent->args[4] == SORCBALL_TERMINAL_SPEED)) {
...
void A_CastSorcererSpell(mobj_t *actor)
{
	...
	case MT_SORCBALL3: // Reinforcements
		...
		if (actor->health <
		    (actor->info->spawnhealth / 3)) { // Spawn 2 at a time
...
void A_SpawnFizzle(mobj_t *actor)
{
	...
	for (ix = 0; ix < 5; ix++) {
		...
		if (mo) {
			...
			mo->momx = FixedMul(P_Random() % speed,
					    finecosine[rangle]);
			mo->momy =
				FixedMul(P_Random() % speed, finesine[rangle]);
			mo->momz = FRACUNIT * 2;
		}
	}
}
		...
		if (dist < CLASS_BOSS_STRAFE_RANGE) {
			if (P_Random() < 100) {
				...
				if (P_Random() < 128)
					...
					ang -= ANGLE_90;
				ang >>= ANGLETOFINESHIFT;
				actor->momx = FixedMul(13 * FRACUNIT,
						       finecosine[ang]);
				actor->momy =
...
void A_FreezeDeath(mobj_t *actor)
{
	...
	actor->flags2 |= MF2_PUSHABLE | MF2_TELESTOMP | MF2_PASSMOBJ |
			 MF2_SLIDE;
	actor->height <<= 2;
	...
	} else if (actor->flags & MF_COUNTKILL &&
		   actor->special) { // Initiate monster death actions
		P_ExecuteLineSpecial(actor->special, actor->args, NULL, 0,
				     actor);
	}
}
...
void A_FreezeDeathChunks(mobj_t *actor)
{
	...
	for (i = 12 + (P_Random() & 15); i >= 0; i--) {
		...
		if (mo) {
			mo->momz = FixedDiv(mo->z - actor->z, actor->height)
				   << 2;
...
void A_KSpiritWeave(mobj_t *actor)
{
	...
	newY = actor->y -
	       FixedMul(finesine[angle], FloatBobOffsets[weaveXY] << 2);
