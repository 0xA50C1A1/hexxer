...
//
//==================================================================
switchlist_t alphSwitchList[] = { { "SW_1_UP", "SW_1_DN", SFX_SWITCH1 },
...
void P_ChangeSwitchTexture(line_t *line, int useAgain)
{
	...
	for (i = 0; i < numswitches * 2; i++) {
		if (switchlist[i] == texTop) {
			...
			if (useAgain) {
				P_StartButton(line, SWTCH_TOP, switchlist[i],
					      BUTTONTIME);
			}
			return;
			...
			if (useAgain) {
				P_StartButton(line, SWTCH_MIDDLE, switchlist[i],
					      BUTTONTIME);
			}
			return;
			...
			sides[line->sidenum[0]].bottomtexture =
				switchlist[i ^ 1];
			if (useAgain) {
				P_StartButton(line, SWTCH_BOTTOM, switchlist[i],
					      BUTTONTIME);
			}
			return;
		}
