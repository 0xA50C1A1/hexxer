...
char *TextKeyMessages[] = { TXT_KEY_STEEL,  TXT_KEY_CAVE,    TXT_KEY_AXE,
			    TXT_KEY_FIRE,   TXT_KEY_EMERALD, TXT_KEY_DUNGEON,
			    TXT_KEY_SILVER, TXT_KEY_RUSTED,  TXT_KEY_HORN,
			    TXT_KEY_SWAMP,  TXT_KEY_CASTLE };
...
boolean P_GiveMana(player_t *player, manatype_t mana, int count)
{
	...
	if (gameskill == sk_baby ||
	    gameskill ==
		    sk_nightmare) { // extra mana in baby mode and nightmare mode
		count += count >> 1;
	}
	...
	if (player->class == PCLASS_FIGHTER &&
	    player->readyweapon == WP_SECOND && mana == MANA_1 &&
	    prevMana <= 0) {
			    ...
			    char *message)
{
	...
	if (player->class !=
		...
		if (netgame &&
		    !deathmatch) { // Can't pick up weapons for other classes in coop netplay
			return;
		}
				 ...
				 int pieceValue, mobj_t *pieceMobj)
{
	...
	boolean gaveWeapon;
	int gaveMana;
	static char *fourthWeaponText[] = { TXT_WEAPON_F4, TXT_WEAPON_C4,
					    TXT_WEAPON_M4 };
	...
	gaveWeapon = false;
	if (player->class !=
	    matchClass) { // Wrong class, but try to pick up for mana
		if (netgame &&
	...
	if (pieceMobj->special) {
		P_ExecuteLineSpecial(pieceMobj->special, pieceMobj->args, NULL,
				     0, player->mo);
		pieceMobj->special = 0;
	}
	if (remove) {
...
boolean P_GivePower(player_t *player, powertype_t power)
{
	...
	if (power == pw_flight) {
		...
		if (player->mo->z <= player->mo->floorz) {
			player->flyheight =
				10; // thrust the player in the air a bit
		}
		return (true);
	}
			      ...
			      mobj_t *artifact)
{
	...
	if (P_GiveArtifact(player, artifactType, artifact)) {
		...
		if (artifactType < arti_firstpuzzitem) {
			...
			P_SetMessage(player, artifactMessages[artifactType],
				     false);
		} else { // Puzzle item
...
boolean P_GiveArtifact(player_t *player, artitype_t arti, mobj_t *mo)
{
	...
	slidePointer = false;
	i = 0;
	while (player->inventory[i].type != arti &&
		...
		if (arti < arti_firstpuzzitem) {
			...
			if (i != player->inventorySlotNum) {
				for (j = player->inventorySlotNum; j > i; j--) {
					...
					player->inventory[j].type =
						player->inventory[j - 1].type;
					slidePointer = true;
				}
			}
		}
...
void P_TouchSpecialThing(mobj_t *special, mobj_t *toucher)
{
	...
	case SPR_KEYB:
		...
		P_SetMessage(player,
			     TextKeyMessages[special->sprite - SPR_KEY1], true);
	...
	case SPR_WMLG: // Arc of Death
		TryPickupWeapon(player, PCLASS_MAGE, WP_THIRD, special,
				TXT_WEAPON_M3);
		return;
...
void P_DamageMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source, int damage)
{
	...
	if (inflictor) {
		...
		case MT_MINOTAUR:
			if (inflictor->flags &
			    MF_SKULLFLY) { // Slam only when in charge mode
		...
		case MT_POISONCLOUD:
			if (target->player) {
				if (target->player->poisoncount < 4) {
					...
					P_PoisonPlayer(target->player, source,
						       50);
					S_StartSound(target,
				...
				return;
				     ...
				     MF_COUNTKILL)) { // only damage monsters/players with the poison cloud
				return;
			}
			break;
	...
	if (player) {
		...
		if (savedPercent) { // armor absorbed some damage
			...
			saved = FixedDiv(FixedMul(damage << FRACBITS,
						  savedPercent),
					 100 * FRACUNIT);
	...
	if (target->health <= 0) { // Death
		if (inflictor) { // check for special fire damage or ice damage deaths
			if (inflictor->flags2 & MF2_FIREDAMAGE) {
				if (player &&
				    !player->morphTics) { // Check for flame death
					if (target->health > -50 &&
					    damage > 25) {
						target->flags2 |=
							MF2_FIREDAMAGE;
		...
		if (source &&
		    (source->type ==
		     MT_MINOTAUR)) { // Minotaur's kills go to his master
