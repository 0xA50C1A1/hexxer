...
{
	if (abs(actor->momx) < 1.5 * FRACUNIT &&
	    abs(actor->momy) < 1.5 * FRACUNIT && actor->momz < 2 * FRACUNIT &&
	...
	if (actor->args[1]-- > 0) {
		for (playnum = 0; playnum < MAXPLAYERS; playnum++) {
			...
			if ((dist < actor->args[2]) &&
				...
				if (P_Random() < 50) {
					P_DamageMobj(victim, NULL, NULL,
						     HITDICE(1));
				}
				// Thrust player around
				an = victim->angle + ANGLE_1 * P_Random();
				P_ThrustMobj(victim, an,
					     richters << (FRACBITS - 1));
			}
		}
	} else {
