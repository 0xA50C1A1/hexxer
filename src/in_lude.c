...
static char *ClusMsgLumpNames[] = { "clus1msg", "clus2msg", "clus3msg",
				    "clus4msg", "clus5msg" };
	...
	if (!deathmatch) {
		...
		if (oldCluster != P_GetMapCluster(LeaveMap)) {
			if (oldCluster >= 1 && oldCluster <= 5) {
				...
				ClusterMessage[msgSize] =
					0; // Append terminator
				HubText = ClusterMessage;
				HubCount =
					strlen(HubText) * TEXTSPEED + TEXTWAIT;
...
static void LoadPics(void)
{
	...
	if (HubCount || gametype == DEATHMATCH) {
		...
		for (i = 0; i < 10; i++) {
			FontBNumbers[i] =
				W_CacheLumpNum(FontBLumpBase + i, PU_STATIC);
		}
...
static void DrDeathTally(void)
{
	...
	if (intertime < TALLY_EFFECT_TICKS) {
		...
		xStart = TALLY_START_XPOS -
			 FixedMul(scale, TALLY_START_XPOS - TALLY_STOP_XPOS);
		yPos = TALLY_START_YPOS -
		       FixedMul(scale, TALLY_START_YPOS - TALLY_STOP_YPOS);
	} else {
	...
	for (i = 0; i < MAXPLAYERS; i++) {
		...
		for (j = 0; j < MAXPLAYERS; j++, xPos += xDelta) {
			...
			if (playeringame[i] && playeringame[j]) {
				if (bold) {
					DrNumberBold(players[i].frags[j], x, y,
						     100);
				} else {
					DrNumber(players[i].frags[j], x, y,
						 100);
				}
			} else {
