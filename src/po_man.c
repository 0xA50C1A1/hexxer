...
void T_PolyDoor(polydoor_t *pd)
{
	...
	case PODOOR_SLIDE:
		if (PO_MovePolyobj(pd->polyobj, pd->xSpeed, pd->ySpeed)) {
			...
			poly = GetPolyobj(pd->polyobj);
			if (poly->crush ||
			    !pd->close) { // continue moving if the poly is a crusher, or is opening
				return;
			} else { // open back up
...
boolean EV_OpenPolyDoor(line_t *line, byte *args, podoortype_t type)
{
	...
	while (mirror = GetPolyobjMirror(polyNum)) {
		...
		if (type == PODOOR_SLIDE) {
			...
			pd->direction = an >> ANGLETOFINESHIFT;
			pd->xSpeed =
				FixedMul(pd->speed, finecosine[pd->direction]);
			pd->ySpeed =
				FixedMul(pd->speed, finesine[pd->direction]);
			SN_StartSequence((mobj_t *)&poly->startSpot,
					 SEQ_DOOR_STONE + poly->seqType);
		} else if (type == PODOOR_SWING) {
			pd->waitTics = args[3];
			...
			pd->speed =
				(args[1] * pd->direction * (ANGLE_90 / 64)) >>
				3;
			pd->totalDist = args[2] * (ANGLE_90 / 64);
			pd->dist = pd->totalDist;
			SN_StartSequence((mobj_t *)&poly->startSpot,
...
static void ThrustMobj(mobj_t *mobj, seg_t *seg, polyobj_t *po)
{
	...
	mobj->momy += thrustY;
	if (po->crush) {
		if (!P_CheckPosition(mobj, mobj->x + thrustX,
				     mobj->y + thrustY)) {
	...
	for (j = po->bbox[BOXBOTTOM]; j <= po->bbox[BOXTOP]; j++) {
		...
		for (i = po->bbox[BOXLEFT]; i <= po->bbox[BOXRIGHT]; i++) {
			if (i >= 0 && i < bmapwidth && j >= 0 &&
				...
				if (link ==
				    NULL) { // polyobj not located in the link cell
					continue;
				}
				link->polyobj = NULL;
			}
		...
		for (i = po->bbox[BOXLEFT]; i <= po->bbox[BOXRIGHT]; i++) {
			if (i >= 0 && i < bmapwidth && j >= 0 &&
				...
				if (!(*link)) { // Create a new link at the current block cell
					*link = Z_Malloc(sizeof(polyblock_t),
							 PU_LEVEL, 0);
					(*link)->next = NULL;
					(*link)->prev = NULL;
					(*link)->polyobj = po;
...
static void SpawnPolyobj(int index, int tag, boolean crush)
{
	...
	for (i = 0; i < numsegs; i++) {
		if (segs[i].linedef->special == PO_LINE_START &&
			...
			polyobjs[index].segs = Z_Malloc(
				PolySegCount * sizeof(seg_t *), PU_LEVEL, 0);
			*(polyobjs[index].segs) =
				&segs[i]; // insert the first seg
	...
	if (!polyobjs[index]
		     .segs) { // didn't find a polyobj through PO_LINE_START
		psIndex = 0;
		polyobjs[index].numsegs = 0;
		for (j = 1; j < PO_MAXPOLYSEGS; j++) {
			psIndexOld = psIndex;
			for (i = 0; i < numsegs; i++) {
				if (segs[i].linedef->special ==
					    PO_LINE_EXPLICIT &&
				    segs[i].linedef->arg1 == tag) {
		...
		if (polyobjs[index].numsegs) {
			PolySegCount =
				polyobjs[index]
					.numsegs; // PolySegCount used globally
			polyobjs[index].crush = crush;
			polyobjs[index].tag = tag;
			polyobjs[index].segs = Z_Malloc(
				polyobjs[index].numsegs * sizeof(seg_t *),
				PU_LEVEL, 0);
			for (i = 0; i < polyobjs[index].numsegs; i++) {
				polyobjs[index].segs[i] = polySegList[i];
			}
			polyobjs[index].seqType =
				(*polyobjs[index].segs)->linedef->arg4;
		}
...
static void TranslateToStartSpot(int tag, int originX, int originY)
{
	...
	for (i = 0; i < po->numsegs; i++, tempSeg++, tempPt++) {
		...
		for (veryTempSeg = po->segs; veryTempSeg != tempSeg;
		     veryTempSeg++) {
		...
		if (veryTempSeg ==
		    tempSeg) { // the point hasn't been translated, yet
			(*tempSeg)->v1->x -= deltaX;
			(*tempSeg)->v1->y -= deltaY;
		}
...
void PO_Init(int lump)
{
	...
	for (i = 0; i < numthings; i++, mt++) {
		...
		if (mt->type == PO_SPAWN_TYPE ||
		    mt->type == PO_SPAWNCRUSH_TYPE) { // Polyobj StartSpot Pt.
	...
	for (i = 0; i < numthings; i++, mt++) {
		...
		mt->angle = SHORT(mt->angle);
		mt->type = SHORT(mt->type);
		if (mt->type == PO_ANCHOR_TYPE) { // Polyobj Anchor Pt.
