...
void P_BringUpWeapon(player_t *player)
{
	...
	if (player->class == PCLASS_FIGHTER &&
	    player->pendingweapon == WP_SECOND && player->mana[MANA_1]) {
		new = S_FAXEUP_G;
	} else {
...
boolean P_CheckMana(player_t *player)
{
	...
	if (mana == MANA_BOTH) {
		if (player->mana[MANA_1] >= count &&
		    player->mana[MANA_2] >= count) {
			return true;
		}
	...
	do {
		if (player->weaponowned[WP_THIRD] &&
		    player->mana[MANA_2] >=
			    WeaponManaUse[player->class][WP_THIRD]) {
...
void P_FireWeapon(player_t *player)
{
	...
	P_SetMobjState(player->mo,
		       PStateAttack[player->class]); // S_PLAY_ATK1);
	if (player->class == PCLASS_FIGHTER &&
...
void AdjustPlayerAngle(mobj_t *pmo)
{
	...
	if (abs(difference) > MAX_ANGLE_ADJUST) {
		pmo->angle += difference > 0 ? MAX_ANGLE_ADJUST :
					       -MAX_ANGLE_ADJUST;
	} else {
		pmo->angle = angle;
	}
...
void A_FSwordAttack(player_t *player, pspdef_t *psp)
{
	...
	P_SPMAngleXYZ(pmo, pmo->x, pmo->y, pmo->z, MT_FSWORD_MISSILE,
		      pmo->angle);
	P_SPMAngleXYZ(pmo, pmo->x, pmo->y, pmo->z + 5 * FRACUNIT,
		      MT_FSWORD_MISSILE, pmo->angle - ANG45 / 8);
	P_SPMAngleXYZ(pmo, pmo->x, pmo->y, pmo->z + 10 * FRACUNIT,
...
void A_LightningClip(mobj_t *actor)
{
	...
	if (target) {
		if (target->health <= 0) {
			...
			actor->momx = 0;
			actor->momy = 0;
			P_ThrustMobj(actor, actor->angle,
				     actor->info->speed >> 1);
		}
	}
}
...
void A_MStaffPalette(player_t *player, pspdef_t *psp)
{
	...
	if (player == &players[consoleplayer]) {
		...
		if (pal ==
		    STARTSCOURGEPAL + 3) { // reset back to original playpal
			pal = 0;
		}
		I_SetPalette((byte *)W_CacheLumpNum(W_GetNumForName("playpal"),
						    PU_CACHE) +
...
void A_CStaffCheck(player_t *player, pspdef_t *psp)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (linetarget) {
			P_LineAttack(pmo, angle, 1.5 * MELEERANGE, slope,
				     damage);
			pmo->angle = R_PointToAngle2(
				pmo->x, pmo->y, linetarget->x, linetarget->y);
			if ((linetarget->player ||
			     linetarget->flags & MF_COUNTKILL) &&
			    (!(linetarget->flags2 &
			       (MF2_DORMANT + MF2_INVULNERABLE)))) {
...
void A_CFlameMissile(mobj_t *actor)
{
	...
	if (BlockingMobj &&
	    BlockingMobj->flags &
		    MF_SHOOTABLE) { // Hit something, so spawn the flame circle around the thing
...
void A_CHolyAttack2(mobj_t *actor)
{
	...
	for (j = 0; j < 4; j++) {
		...
		mo->z = actor->z;
		mo->angle =
			actor->angle + (ANGLE_45 + ANGLE_45 / 2) - ANGLE_45 * j;
...
static void CHolyTailFollow(mobj_t *actor, fixed_t dist)
{
	...
	if (child) {
		an = R_PointToAngle2(actor->x, actor->y, child->x, child->y) >>
		     ANGLETOFINESHIFT;
		oldDistance = P_AproxDistance(child->x - actor->x,
					      child->y - actor->y);
...
void A_CHolyTail(mobj_t *actor)
{
	...
	if (parent) {
		if (parent->state >=
		    &states[parent->info->deathstate]) { // Ghost removed, so remove all tail parts
			...
			return;
				   ...
				   parent->x -
					   FixedMul(
						   14 * FRACUNIT,
						   finecosine[parent->angle >>
							      ANGLETOFINESHIFT]),
				   parent->y -
					   FixedMul(
						   14 * FRACUNIT,
						   finesine[parent->angle >>
							    ANGLETOFINESHIFT]))) {
...
void A_ShedShard(mobj_t *actor)
{
	...
	if (spawndir & SHARDSPAWN_UP) {
		mo = P_SpawnMissileAngleSpeed(
			actor, MT_SHARDFX1, actor->angle, 0,
