...
void SN_InitSequenceScript(void)
{
	...
	while (SC_GetString()) {
		if (*sc_String == ':') {
			if (inSequence != -1) {
				SC_ScriptError(
					"SN_InitSequenceScript:  Nested Script Error");
			}
			...
			for (j = 0; j < SEQ_NUMSEQ; j++) {
				if (!strcasecmp(SequenceTranslate[j].name,
						sc_String + 1)) {
		...
		if (SC_Compare(SS_STRING_PLAYUNTILDONE)) {
			...
			*tempDataPtr++ = SS_CMD_END;
			...
			SequenceData[i] =
				(int *)Z_Malloc(dataSize, PU_STATIC, NULL);
			...
			*tempDataPtr++ = SS_CMD_STOPSOUND;
			...
			SC_ScriptError(
				"SN_InitSequenceScript:  Unknown commmand.\n");
		}
	}
}
...
void SN_StopSequence(mobj_t *mobj)
{
	...
	for (node = SequenceListHead; node; node = node->next) {
		if (node->mobj == mobj) {
			S_StopSound(mobj);
			if (node->stopSound) {
				S_StartSoundAtVolume(mobj, node->stopSound,
						     node->volume);
			}
...
void SN_UpdateActiveSequences(void)
{
	...
	for (node = SequenceListHead; node; node = node->next) {
		...
		sndPlaying =
			S_GetSoundPlayingInfo(node->mobj, node->currentSoundID);
		switch (*node->sequencePtr) {
		case SS_CMD_PLAY:
			if (!sndPlaying) {
				node->currentSoundID = *(node->sequencePtr + 1);
				S_StartSoundAtVolume(node->mobj,
						     node->currentSoundID,
						     node->volume);
			}
			node->sequencePtr += 2;
