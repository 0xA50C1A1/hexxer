...
*/
	...
	if (!ld->backsector) { // One sided line
		if (tmthing->flags2 & MF2_BLASTED) {
			P_DamageMobj(tmthing, NULL, NULL,
				     tmthing->info->mass >> 5);
		}
...
boolean PIT_CheckThing(mobj_t *thing)
{
	...
	if (tmthing->flags2 &
		...
		if (tmthing->type == MT_BISHOP &&
		    thing->type ==
			    MT_BISHOP) { // don't let bishops fly over other bishops
			return false;
		}
		...
		if (tmthing->type == MT_MINOTAUR) {
			...
			if (thing->flags & MF_SHOOTABLE &&
				...
				if (P_Random() < 96) {
					...
					P_DamageMobj(thing, tmthing,
						     tmthing->target, damage);
					if (P_Random() < 128) {
						P_SpawnMobj(tmthing->x,
							    tmthing->y,
							    tmthing->z,
							    MT_HOLY_PUFF);
						S_StartSound(tmthing,
							     SFX_SPIRIT_ATTACK);
						if (thing->flags &
							    MF_COUNTKILL &&
						    P_Random() < 128 &&
	...
	if (tmthing->flags & MF_MISSILE) {
		...
		} else if (tmthing->type == MT_MSTAFF_FX2 &&
			   thing != tmthing->target) {
		...
		if (tmthing->target &&
			...
			if (!thing->player) { // Hit same species as originator, explode, no damage
				return (false);
			}
		}
		...
		if (tmthing->flags2 & MF2_RIP) {
			if (!(thing->flags & MF_NOBLOOD) &&
			    ...
			    !(thing->flags2 &
			      MF2_INVULNERABLE)) { // Ok to spawn some blood
		...
		if (damage) {
			if (!(thing->flags & MF_NOBLOOD) &&
			    ...
			    (P_Random() < 192)) {
				P_BloodSplatter(tmthing->x, tmthing->y,
						tmthing->z, thing);
			}
			P_DamageMobj(thing, tmthing, tmthing->target, damage);
		}
...
boolean P_TryMove(mobj_t *thing, fixed_t x, fixed_t y)
{
	...
	if (!(thing->flags & MF_NOCLIP)) {
		...
		if (!(thing->flags & MF_TELEPORT) &&
		    ...
		    !(thing->flags2 &
		      MF2_FLY)) { // mobj must lower itself to fit
			goto pushline;
		}
...
*/
	...
	if (la_damage) {
		...
		if (PuffType ==
		    MT_FLAMEPUFF2) { // Cleric FlameStrike does fire damage
...
boolean PTR_PuzzleItemTraverse(intercept_t *in)
{
	...
	if (in->isaline) { // Check line
		...
		if (P_PointOnLineSide(PuzzleItemUser->x, PuzzleItemUser->y,
				      in->d.line) ==
		    1) { // Don't use back sides
			return false;
		}
		if (PuzzleItemType !=
		    in->d.line->arg1) { // Item type doesn't match
			return false;
		}
...
boolean PIT_RadiusAttack(mobj_t *thing)
{
	...
	if (P_CheckSight(thing,
			 bombspot)) { // OK to damage, target is in direct path
		damage =
