...
void P_InitTerrainTypes(void)
{
	...
	for (i = 0; TerrainTypeDefs[i].type != -1; i++) {
		...
		if (lump != -1) {
			TerrainTypes[lump - firstflat] =
				TerrainTypeDefs[i].type;
		}
	}
}
			     ...
			     mobj_t *mo)
{
	...
	case 13: // Door Locked_Raise
		if (CheckedLockedDoor(mo, args[3])) {
			if (!args[0]) {
				...
				buttonSuccess =
					EV_DoDoor(line, args, DREV_NORMAL);
			}
		}
		break;
	...
	case 25: // Floor Raise to Nearest
		buttonSuccess =
			EV_DoFloor(line, args, FLEV_RAISEFLOORTONEAREST);
		break;
	...
	case 45: // Ceiling Crush Raise and Stay
		buttonSuccess =
			EV_DoCeiling(line, args, CLEV_CRUSHRAISEANDSTAY);
		break;
	...
	case 63: // Plat Down-by-Value*8-Wait-Up-Stay
		buttonSuccess =
			EV_DoPlat(line, args, PLAT_DOWNBYVALUEWAITUPSTAY, 0);
		break;
	...
	case 65: // Plat Up-by-Value*8-Wait-Down-Stay
		buttonSuccess =
			EV_DoPlat(line, args, PLAT_UPBYVALUEWAITDOWNSTAY, 0);
		break;
	...
	case 69: // Ceiling Move to Value * 8
		buttonSuccess =
			EV_DoCeiling(line, args, CLEV_MOVETOVALUETIMES8);
		break;
	case 70: // Teleport
		if (side ==
		    0) { // Only teleport when crossing the front side of a line
...
boolean P_ActivateLine(line_t *line, mobj_t *mo, int side, int activationType)
{
	...
	if (!mo->player && !(mo->flags & MF_MISSILE)) {
		if (lineActivation !=
		    SPAC_MCROSS) { // currently, monsters can only activate the MCROSS activation type
			return false;
		}
...
void P_PlayerInSpecialSector(player_t *player)
{
	...
	case 221: // Scroll_SouthEast_xxx
		P_Thrust(player, ANG270 + ANG45,
			 pushTab[sector->special - 219]);
		break;
	case 222:
	case 223:
	case 224: // Scroll_SouthWest_xxx
		P_Thrust(player, ANG180 + ANG45,
			 pushTab[sector->special - 222]);
		break;
...
{
	if (player->mo->z !=
	    player->mo->floorz) { // Player is not touching the floor
		return;
	}
	switch (floorType) {
	case FLOOR_LAVA:
