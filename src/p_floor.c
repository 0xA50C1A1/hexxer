			       ...
			       stairs_e stairsType, int delay, int resetDelay)
{
	...
	for (i = 0; i < sec->linecount; i++) {
		...
		if (tsec->special == type + STAIR_SECTOR_TYPE &&
		    !tsec->specialdata && tsec->floorpic == Texture &&
...
int EV_BuildStairs(line_t *line, byte *args, int direction, stairs_e stairsType)
{
	int secnum;
	int height;
	int delay;
	...
	while ((qSec = DequeueStairSector(&type, &height)) != NULL) {
		ProcessStairSector(qSec, type, height, stairsType, delay,
				   resetDelay);
	}
	return (1);
}
...
int EV_BuildPillar(line_t *line, byte *args, boolean crush)
{
	...
	while ((secnum = P_FindSectorFromTag(args[0], secnum)) >= 0) {
		...
		if (sec->floorheight ==
		    sec->ceilingheight) { // pillar is already closed
			continue;
		}
		rtn = 1;
		if (!args[2]) {
...
int EV_OpenPillar(line_t *line, byte *args)
{
	...
	while ((secnum = P_FindSectorFromTag(args[0], secnum)) >= 0) {
		...
		if (sec->specialdata)
			continue; // already moving
		if (sec->floorheight !=
		    sec->ceilingheight) { // pillar isn't closed
			continue;
		}
		rtn = 1;
		pillar = Z_Malloc(sizeof(*pillar), PU_LEVSPEC, 0);
		...
		if (!args[2]) {
			...
			pillar->floordest =
				sec->floorheight - (args[2] << FRACBITS);
		}
		if (!args[3]) {
			pillar->ceilingdest =
			...
			pillar->ceilingdest =
				sec->ceilingheight + (args[3] << FRACBITS);
		}
...
void T_FloorWaggle(floorWaggle_t *waggle)
{
	...
	waggle->sector->floorheight =
		...
		FixedMul(
			FloatBobOffsets[(waggle->accumulator >> FRACBITS) & 63],
			waggle->scale);
	P_ChangeSector(waggle->sector, true);
}
