...
void P_XYMovement(mobj_t *mo)
{
	...
	if (!mo->momx && !mo->momy) {
		if (mo->flags &
		    MF_SKULLFLY) { // A flying mobj slammed into something
...
explode:
				...
				if (ceilingline && ceilingline->backsector &&
				    ceilingline->backsector->ceilingpic ==
					    skyflatnum) { // Hack to prevent missiles exploding against the sky
	...
	if (player &&
	    player->cheats &
		    CF_NOMOMENTUM) { // Debug option for no sliding at all
		mo->momx = mo->momy = 0;
		return;
	}
	if (mo->flags &
	    (MF_MISSILE | MF_SKULLFLY)) { // No friction for missiles
		return;
	}
	...
	if (mo->flags &
	    MF_CORPSE) { // Don't stop sliding if halfway off a step with some momentum
...
void P_ZMovement(mobj_t *mo)
{
	...
	if (mo->player && mo->flags2 & MF2_FLY && !(mo->z <= mo->floorz) &&
	    leveltime & 2) {
		mo->z +=
			finesine[(FINEANGLES / 20 * leveltime >> 2) & FINEMASK];
	}
	...
	if (mo->z <= mo->floorz) { // Hit the floor
		if (mo->flags & MF_MISSILE) {
			...
			if (mo->flags2 & MF2_FLOORBOUNCE) {
				...
				return;
				   ...
				   MT_HOLY_FX) { // The spirit struck the ground
				mo->momz = 0;
				P_HitFloor(mo);
				return;
			} else if (mo->type == MT_MNTRFX2 ||
				   mo->type ==
					   MT_LIGHTNING_FLOOR) { // Minotaur floor fire can go up steps
				return;
			} else {
				P_HitFloor(mo);
				P_ExplodeMissile(mo);
		...
		if (mo->momz < 0) {
			...
			if (mo->player) {
				mo->player->jumpTics =
					7; // delay any jumping for a short time
				if (mo->momz < -GRAVITY * 8 &&
					...
					} else if ((P_GetThingFloorType(mo) <
						    FLOOR_LIQUID) &&
						   (!mo->player->morphTics)) {
						S_StartSound(mo,
							     SFX_PLAYER_LAND);
		...
		if (mo->flags &
		    MF_SKULLFLY) { // The skull slammed into something
			mo->momz = -mo->momz;
		}
...
void P_BlasterMobjThinker(mobj_t *mobj)
{
	...
	// Handle movement
	if (mobj->momx || mobj->momy || (mobj->z != mobj->floorz) ||
	    mobj->momz) {
		xfrac = mobj->momx >> 3;
		yfrac = mobj->momy >> 3;
		zfrac = mobj->momz >> 3;
		...
		for (i = 0; i < 8; i++) {
			...
			if (mobj->z + mobj->height >
			    mobj->ceilingz) { // Hit the ceiling
			...
			if (changexy) {
				if (mobj->type == MT_MWAND_MISSILE &&
					...
					mo = P_SpawnMobj(mobj->x, mobj->y, z,
							 MT_CFLAMEFLOOR);
					if (mo) {
						mo->angle = mobj->angle;
					}
	...
	if (mobj->tics != -1) {
		...
		while (!mobj->tics) {
			if (!P_SetMobjState(
				    mobj,
				    mobj->state->nextstate)) { // mobj was removed
				return;
			}
		}
	}
...
mobj_t *P_SpawnMobj(fixed_t x, fixed_t y, fixed_t z, mobjtype_t type)
{
	...
	if (z == ONFLOORZ) {
		...
		space = ((mobj->ceilingz) - (mobj->info->height)) -
			mobj->floorz;
		if (space > 48 * FRACUNIT) {
			space -= 40 * FRACUNIT;
			mobj->z = ((space * P_Random()) >> 8) + mobj->floorz +
				  40 * FRACUNIT;
		} else {
			mobj->z = mobj->floorz;
		}
...
void P_SpawnPlayer(mapthing_t *mthing)
{
	...
	if (!playeringame[mthing->type - 1]) { // Not playing
		return;
	}
...
void P_SpawnMapThing(mapthing_t *mthing)
{
	...
	mobj_t *mobj;
	fixed_t x, y, z;
	static unsigned int classFlags[] = { MTF_FIGHTER, MTF_CLERIC,
					     MTF_MAGE };
...
int P_HitFloor(mobj_t *thing)
{
	...
	if (thing->floorz !=
	    thing->subsector->sector
		    ->floorheight) { // don't splash if landing on the edge above water/lava/etc....
		return (FLOOR_SOLID);
	}
