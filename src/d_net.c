...
void D_ArbitrateNetStart(void)
{
	...
	do {
		...
		CheckAbort();
		while (HGetPacket()) { // Check for any incoming packets
			if (netbuffer->checksum & NCMD_SETUP &&
			    netbuffer->starttic >= 64) {
				PlayerClass[netbuffer->player] =
...
#ifdef __WATCOMC__
		if (ticcount <=
		    nextTic) { // only send packets every half second
			continue;
		}
		nextTic = ticcount + 8;
#endif
		...
		for (i = 0; i < doomcom->numnodes; i++) {
			...
			netbuffer->starttic =
				PlayerClass[doomcom->consoleplayer] + 64;
			netbuffer->retransmitfrom =
				gotinfo[doomcom->consoleplayer];
			netbuffer->numtics = 0;
			HSendPacket(i, NCMD_SETUP);
		}
		for (i = 0; i < doomcom->numnodes;
		     i++) { // Make sure that all nodes have sent class info
			if (!gotClass[i]) {
				ST_Message(".");
				break;
			}
		...
		if (i < doomcom->numnodes) {
			continue;
		} else { // consoleplayer has received all player classes
		...
		for (i = 0; i < doomcom->numnodes;
		     i++) { // Make sure that all nodes are ready to proceed
			if (!gotinfo[i]) {
				break;
			}
		}
	...
	if (doomcom->consoleplayer) { // listen for setup info from key player
	...
	} else { // key player, send the setup info
...
void TryRunTics(void)
{
	...
	if (!demoplayback) {
		...
		for (i = 0; i < MAXPLAYERS; i++)
			if (playeringame[i])
				break;
		if (consoleplayer == i) { // the key player does not adapt
		} else {
