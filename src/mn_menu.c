...
static Menu_t FilesMenu = {
	110, 60, DrawFilesMenu, 2, FilesItems, 0, MENU_MAIN
};
...
void MN_DrTextAYellow(char *text, int x, int y)
{
	...
	while ((c = *text++) != 0) {
		if (c < 33) {
			...
			p = W_CacheLumpNum(FontAYellowBaseLump + c - 33,
					   PU_CACHE);
			V_DrawPatch(x, y, p);
			x += p->width - 1;
		}
...
char *QuitEndMsg[] = { "ARE YOU SURE YOU WANT TO QUIT?",
	...
	if (MenuActive == false) {
		if (askforquit) {
			MN_DrTextA(
				...
				160 - MN_TextAWidth(QuitEndMsg[typeofask - 1]) /
						2,
				80);
			if (typeofask == 3) {
				MN_DrTextA(
					SlotText[quicksave - 1],
					160 - MN_TextAWidth(
						      SlotText[quicksave - 1]) /
							2,
					90);
...
boolean MN_Responder(event_t *event)
{
	...
	if (MenuActive == false && !chatmodeon) {
		...
		case KEY_F2: // save game
			if (gamestate == GS_LEVEL && !demoplayback) {
				...
				S_StartSound(NULL, SFX_DOOR_LIGHT_CLOSE);
				slottextloaded =
					false; //reload the slot text, when needed
			}
			return true;
	...
	if (MenuActive == false) {
		if (key == KEY_ESCAPE || gamestate == GS_DEMOSCREEN ||
		    demoplayback) {
			MN_ActivateMenu();
			return (true);
		}
	...
	if (!FileMenuKeySteal) {
		...
		case KEY_DOWNARROW:
			...
			} while (CurrentMenu->items[CurrentItPos].type ==
				 ITT_EMPTY);
...
static void DrawSlider(Menu_t *menu, int item, int width, int slot)
{
	...
	for (x2 = x, count = width; count--; x2 += 8) {
		V_DrawPatch(x2, y,
			    W_CacheLumpName(count & 1 ? "M_SLDMD1" : "M_SLDMD2",
					    PU_CACHE));
	}
