...
void R_InitSpriteDefs(char **namelist)
{
	...
	for (i = 0; i < numsprites; i++) {
		...
		sprites[i].spriteframes = Z_Malloc(
...
void R_DrawVisSprite(vissprite_t *vis, int x1, int x2)
{
	...
	if (vis->psprite) {
		dc_texturemid += FixedMul(
			((centery - viewheight / 2) << FRACBITS), vis->xiscale);
	...
	if (vis->floorclip && !vis->psprite) {
		...
		baseclip =
			(sprbotscreen - FixedMul(vis->floorclip, spryscale)) >>
			FRACBITS;
	} else {
		baseclip = -1;
	}
...
//
	...
	if ((psp->state->frame & FF_FRAMEMASK) >= sprdef->numframes)
		I_Error("R_ProjectSprite: invalid sprite frame %u : %td ",
	...
	tx -= spriteoffset[lump];
	if (viewangleoffset) {
		tempangle = ((centerxfrac / 1024) *
			     (viewangleoffset >> ANGLETOFINESHIFT));
	} else {
		tempangle = 0;
	}
	...
	if (x1 > viewwidth)
		return; // off the right side
	tx += spritewidth[lump];
	x2 = ((centerxfrac + FixedMul(tx, pspritescale) + tempangle) >>
	      FRACBITS) -
	     1;
	if (x2 < 0)
		return; // off the left side
	...
	if (viewplayer->powers[pw_invulnerability] &&
		...
		if (viewplayer->powers[pw_invulnerability] > 4 * 32) {
			if (viewplayer->mo->flags2 &
			    MF2_DONTDRAW) { // don't draw the psprite
