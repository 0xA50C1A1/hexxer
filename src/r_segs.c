...
void R_RenderSegLoop(void)
{
	...
	for (; rw_x < rw_stopx; rw_x++) {
		...
		if (midtexture) { // single sided line
			...
			if (maskedtexture) { // save texturecol for backdrawing of masked mid texture
...
void R_StoreWallRange(int start, int stop)
{
	...
	if (!backsector) {
		...
		if (linedef->flags & ML_DONTPEGBOTTOM) {
			...
			rw_midtexturemid =
				vtop - viewz; // bottom of texture at bottom
		} else
		...
		if (worldhigh < worldtop) { // top texture
			...
			if (linedef->flags & ML_DONTPEGTOP)
				rw_toptexturemid =
					worldtop; // top of texture at top
			else {
				vtop = backsector->ceilingheight +
				       textureheight[sidedef->toptexture];
				rw_toptexturemid =
					vtop - viewz; // bottom of texture
			}
		}
		if (worldlow > worldbottom) { // bottom texture
			...
			if (linedef->flags &
			    ML_DONTPEGBOTTOM) { // bottom of texture at bottom
				rw_bottomtexturemid =
					worldtop; // top of texture at top
	...
	if (backsector) {
		...
		if (worldlow > worldbottom) {
			pixlow = (centeryfrac >> 4) -
				 FixedMul(worldlow, rw_scale);
