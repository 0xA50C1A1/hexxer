...
void P_CalcHeight(player_t *player)
{
	...
	if (player->morphTics) {
		player->viewz =
			player->mo->z + player->viewheight - (20 * FRACUNIT);
	} else {
	...
	if (fly && player->powers[pw_flight]) {
		if (fly != TOCENTER) {
			...
			if (!(player->mo->flags2 & MF2_FLY)) {
				...
				if (player->mo->momz <=
				    -39 * FRACUNIT) { // stop falling scream
...
void P_DeathThink(player_t *player)
{
	...
	if (player->attacker &&
	    player->attacker != player->mo) { // Watch killer
		dir = P_FaceMobj(player->mo, player->attacker, &delta);
		if (delta <
		    ANGLE_1 *
			    10) { // Looking at killer, so fade damage and poison counters
...
void P_PlayerThink(player_t *player)
{
	...
	if (player->mo->reactiontime) { // Player is frozen
		...
		if (player->powers[pw_speed] && !(leveltime & 1) &&
			...
			speedMo = P_SpawnMobj(pmo->x, pmo->y, pmo->z,
					      MT_PLAYER_SPEED);
			if (speedMo) {
	...
	if (player->powers[pw_invulnerability]) {
		...
		if (!(--player->powers[pw_invulnerability])) {
			player->mo->flags2 &=
				~(MF2_INVULNERABLE | MF2_REFLECTIVE);
			if (player->class == PCLASS_CLERIC) {
				player->mo->flags2 &=
					~(MF2_DONTDRAW | MF2_NONSHOOTABLE);
				player->mo->flags &=
					~(MF_SHADOW | MF_ALTSHADOW);
			}
		}
	}
	...
	if (player->powers[pw_flight] && netgame) {
		if (!--player->powers[pw_flight]) {
			...
			player->mo->flags &= ~MF_NOGRAVITY;
			BorderTopRefresh =
				true; //make sure the sprite's cleared out
		}
	}
...
void P_BlastMobj(mobj_t *source, mobj_t *victim, fixed_t strength)
{
	...
	} else // full strength blast from artifact
	{
		...
		// Spawn blast puff
		ang = R_PointToAngle2(victim->x, victim->y, source->x,
				      source->y);
...
void P_BlastRadius(player_t *player)
{
	...
	for (think = thinkercap.next; think != &thinkercap;
		...
		mo = (mobj_t *)think;
		if ((mo == pmo) ||
		    (mo->flags2 & MF2_BOSS)) { // Not a valid monster
			continue;
		}
...
void P_PlayerRemoveArtifact(player_t *player, int slot)
{
	...
	if (!(--player->inventory[slot]
		...
		if (player ==
		    &players[consoleplayer]) { // Set position markers and get next readyArtifact
			inv_ptr--;
			if (inv_ptr < 6) {
				curpos--;
				if (curpos < 0) {
		...
		if (player->inventory[i].type ==
			...
			if (P_UseArtifact(
				...
				if (player == &players[consoleplayer]) {
				   ...
				   arti_firstpuzzitem) { // Unable to use artifact, advance pointer
...
boolean P_UseArtifact(player_t *player, artitype_t arti)
{
		...
		} else // PCLASS_FIGHTER, obviously (also pig, not so obviously)
		{
			...
			if (mo) {
				mo->angle = player->mo->angle +
					    (((P_Random() & 7) - 4) << 24);
				mo->momz = 4 * FRACUNIT + ((player->lookdir)
							   << (FRACBITS - 4));
	...
	case arti_boostarmor:
		...
		for (i = 0; i < NUMARMOR; i++) {
			count += P_GiveArmor(player, i,
					     1); // 1 point per armor type
		}
		if (!count) {
			return false;
		}
