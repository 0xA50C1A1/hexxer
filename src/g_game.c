...
void G_BuildTiccmd(ticcmd_t *cmd)
{
...
#ifdef __WATCOMC__
	if (useexterndriver && look != TOCENTER &&
		...
		if (gamestate == GS_LEVEL) {
			if (externInvKey & EBT_MAP) { // automap
				ev.type = ev_keyup;
	...
	if (gamekeydown[key_flyup]) {
		flyheight =
			5; // note that the actual flyheight will be twice this
	}
	...
	if (gamekeydown[key_useartifact]) {
		if (gamekeydown[key_speed] && artiskip) {
			...
			if (inventory) {
				...
				cmd->arti |= players[consoleplayer]
						     .inventory[inv_ptr]
						     .type &
					     AFLAG_MASK;
				usearti = false;
...
boolean G_Responder(event_t *ev)
{
	...
	if (gamestate == GS_LEVEL && ev->type == ev_keydown &&
	    ev->data1 == KEY_F12 && !deathmatch) { // Cycle the display player
		do {
			displayplayer++;
...
boolean G_CheckSpot(int playernum, mapthing_t *mthing)
{
	...
	if (players[consoleplayer].viewz != 1)
		S_StartSound(mo,
			     SFX_TELEPORT); // don't start sound on first frame
...
void G_DoReborn(int playernum)
{
	...
	if (!netgame) {
		if (SV_RebornSlotAvailable()) { // Use the reborn code if the slot is available
		...
		if (G_CheckSpot(
			    playernum,
			    &playerstarts[RebornPosition]
					 [playernum])) { // Appropriate player start spot is open
			...
			for (i = 0; i < MAXPLAYERS; i++) {
				if (G_CheckSpot(
					    playernum,
					    &playerstarts[RebornPosition]
							 [i])) { // Found an open start spot

					// Fake as other player
					playerstarts[RebornPosition][i].type =
						playernum + 1;
					P_SpawnPlayer(
						&playerstarts[RebornPosition]
							     [i]);
		...
		if (foundSpot ==
		    false) { // Player's going to be inside something
			P_SpawnPlayer(&playerstarts[RebornPosition][playernum]);
		}
