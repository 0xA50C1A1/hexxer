	...
	while (polyLink) {
		if (polyLink->polyobj) {
			if (polyLink->polyobj->validcount != validcount) {
				...
				for (i = 0; i < polyLink->polyobj->numsegs;
				     i++, tempSeg++) {
					if ((*tempSeg)->linedef->validcount ==
					    validcount) {
						continue;
					}
					(*tempSeg)->linedef->validcount =
						validcount;
...
mobj_t *P_RoughMonsterSearch(mobj_t *mo, int distance)
{
	...
	if (startX >= 0 && startX < bmapwidth && startY >= 0 &&
	    startY < bmapheight) {
		if (target = RoughBlockCheck(
			    mo, startY * bmapwidth +
					startX)) { // found a target right away
			return target;
		}
	}
	for (count = 1; count <= distance; count++) {
		...
		for (blockIndex--; blockIndex <= secondStop;
		     blockIndex += bmapwidth) {
			if (target = RoughBlockCheck(mo, blockIndex)) {
				return target;
			}
		}
		// Trace the third block section (bottom edge)
		for (blockIndex -= bmapwidth; blockIndex >= thirdStop;
		     blockIndex--) {
			if (target = RoughBlockCheck(mo, blockIndex)) {
				return target;
			}
		...
		} else if (mo->type == MT_MSTAFF_FX2) // bloodscourge
		{
			if ((link->flags & MF_COUNTKILL ||
				...
				if (netgame && !deathmatch && link->player) {
					...
					master = mo->target;
					angle = R_PointToAngle2(master->x,
								master->y,
								link->x,
								link->y) -
						master->angle;
